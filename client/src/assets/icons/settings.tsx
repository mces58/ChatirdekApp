...
const TranslateIcon: React.FC<SettingIconProps> = (props) => {
  ...
  return (
    <View style={[{ width, height }]}>
      <Svg width="100%" height="100%" viewBox="0 0 64 64" fill={color} opacity={opacity}>
        <Path
...
const PaletteIcon: React.FC<SettingIconProps> = (props) => {
  const { width, height, strokeWidth = 4, opacity = 1 } = props;
  const { theme } = useTheme();
...
const ChatSettingIcon: React.FC<SettingIconProps> = (props) => {
...
const ViewOffIcon: React.FC<SettingIconProps> = (props) => {
...
const InfoIcon: React.FC<SettingIconProps> = (props) => {
  const { width, height, strokeWidth = 1, opacity = 1 } = props;
  const { theme } = useTheme();
