...
import prepareFilePath from 'src/utils/preparePath.util';
...
export const getLastMessages = async (req, res) => {
  ...
  try {
    ...
    const conversations = await Conversation.find({
      ...
      .populate({
        path: 'messages',
        select: 'senderId receiverId message image createdAt audio',
      })
      .populate({
        path: 'participants',
        model: 'User',
...
export const sendImageMessage = async (req, res) => {
  ...
  const filePath = prepareFilePath('png');
...
export const sendAudioMessage = async (req, res) => {
  ...
  const filePath = prepareFilePath('mp4');
  ...
  try {
    ...
    removeLocalImage(convertedAudioPath);
  } catch (error) {
    handleErrors(res, error);
  }
};
